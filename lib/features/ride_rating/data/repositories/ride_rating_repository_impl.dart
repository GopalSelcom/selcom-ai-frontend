...
class RideRatingRepositoryImpl implements RideRatingRepository {
  ...
  @override
  Future<Either<Failure, PendingReview?>> getLastCompletedRide() async {
    try {
  ...
  Future<Either<Failure, List<ReviewTagModel>>> getReviewTags({
    ...
    try {
      final response = await remoteDataSource.getReviewTags(rating: rating);
      return Right(response.activeTags);
    } catch (e, stackTrace) {
      ErrorReporter.instance.report(error: e, stackTrace: stackTrace);
      return Left(ServerFailure(e.toString()));
    }
